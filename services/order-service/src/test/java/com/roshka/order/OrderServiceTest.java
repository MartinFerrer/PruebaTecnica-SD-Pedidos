...
class OrderServiceTest {
  ...
  void creationConfirmationCancellationAndRelease() {
    ...
    service.apply(
        new ApplyReservationResultUseCase.Reserved(
            id, 1, reservationItems(service.findById(id).items())));
    ...
    var release =
        new CompleteInventoryCancellationUseCase.Result(
            id, 3, "RELEASED", releaseItems(service.findById(id).items()));
  ...
  void missingOrderAndInvalidResultAreRejected() {
    ...
    assertThatThrownBy(
            () ->
                service.apply(
                    new ApplyReservationResultUseCase.Reserved(id, 5, List.of())))
  ...
  void rejectionPreservesAllShortages() {
    ...
    var shortages =
        List.of(
            new ApplyReservationResultUseCase.Shortage(
                product, 2, 0, "INSUFFICIENT_STOCK"));
  ...
  private void create(UUID productId, long quantity) {
    service.create(
        new CreateOrderUseCase.Command(
            List.of(new CreateOrderUseCase.Item(productId, quantity))));
  }

  private static List<ApplyReservationResultUseCase.Item> reservationItems(
  ...
  private static List<CompleteInventoryCancellationUseCase.Item> releaseItems(
      List<Order.Item> items) {
    return items.stream()
        .map(
            item ->
                new CompleteInventoryCancellationUseCase.Item(
