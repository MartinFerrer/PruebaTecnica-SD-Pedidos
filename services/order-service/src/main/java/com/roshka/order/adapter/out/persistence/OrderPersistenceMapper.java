...
class OrderPersistenceMapper {
	...
	Order toDomain(OrderEntity entity) {
		return new Order(entity.orderId,
				entity.items.stream().map(item -> new Order.Item(item.productId, item.quantity)).toList(),
				OrderStatus.valueOf(entity.status), entity.version, entity.inventoryCancellationStatus,
				entity.cancellationVersion, List.of(json.read(entity.unavailableItems, Order.Shortage[].class)));
	}

	void updateEntity(Order order, OrderEntity entity, boolean fresh) {
		if (fresh) {
			entity.orderId = order.orderId();
			entity.items = new ArrayList<>(order.items()
				.stream()
