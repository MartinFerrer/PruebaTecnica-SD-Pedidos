...
public class OrderApiMapper {
	...
	public CreateOrderUseCase.Command toCommand(OrderRequest.Create request) {
		return new CreateOrderUseCase.Command(request.items()
			.stream()
	...
	public OrderResponse toResponse(Order order) {
		return new OrderResponse(order.orderId(), order.status().name(),
				order.items().stream().map(item -> new OrderResponse.Line(item.productId(), item.quantity())).toList(),
				order.unavailableItems()
					.stream()
					.map(shortage -> new OrderResponse.Shortage(shortage.productId(), shortage.requested(),
							shortage.available(), shortage.reason()))
					.toList(),
				order.inventoryCancellationStatus(), "/orders/" + order.orderId());
	}
