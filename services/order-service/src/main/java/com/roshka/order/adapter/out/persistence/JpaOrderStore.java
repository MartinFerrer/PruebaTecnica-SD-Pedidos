...
public class JpaOrderStore implements OrderStore {
	...
	public Optional<Order> find(UUID id, boolean lock) {
		OrderEntity entity = em.find(OrderEntity.class, id, lock ? LockModeType.PESSIMISTIC_WRITE : LockModeType.NONE);
		return entity == null ? Optional.empty() : Optional.of(mapper.toDomain(entity));
	}

	public List<Order> findAll() {
		return em.createQuery("select o from OrderEntity o order by o.orderId", OrderEntity.class)
			.getResultList()
			.stream()
			.map(mapper::toDomain)
			.toList();
