...
public class OrderService
    implements CreateOrderUseCase,
        FindOrdersQuery,
        CancelOrderUseCase,
        ApplyReservationResultUseCase,
        CompleteInventoryCancellationUseCase {
    ...
    Order after =
        switch (result) {
          ...
          case ApplyReservationResultUseCase.Rejected rejected -> {
            List<Order.Shortage> shortages =
                rejected.shortages().stream()
                    .map(
                        shortage ->
                            new Order.Shortage(
                                shortage.productId(),
  ...
  public void complete(CompleteInventoryCancellationUseCase.Result result) {
    Order before = find(result.orderId(), true);
    List<Order.Item> releasedItems =
        result.releasedItems().stream()
            .map(item -> new Order.Item(item.productId(), item.quantity()))
            .toList();
  ...
  private Order find(UUID orderId, boolean lock) {
    return store.find(orderId, lock)
        ...
        || shortages.stream()
            .anyMatch(
                shortage -> {
                  ...
                  return item == null
                      || shortage.requested() != item.quantity()
                      || shortage.available() < 0
                      || shortage.available() >= shortage.requested()
                      || !Set.of("INSUFFICIENT_STOCK", "PRODUCT_NOT_FOUND")
                          .contains(shortage.reason())
                      || ("PRODUCT_NOT_FOUND".equals(shortage.reason())
                          && shortage.available() != 0);
                })) {
  ...
  private static void validateRelease(
      Order order, String outcome, List<Order.Item> releasedItems) {
    boolean released = "RELEASED".equals(outcome);
    boolean noReservation =
        "NOT_RESERVED".equals(outcome) || "CANCELLED_BEFORE_RESERVATION".equals(outcome);
    boolean itemsMatch =
        releasedItems.size() == order.items().size()
            && new HashSet<>(releasedItems).equals(new HashSet<>(order.items()));
    if ((!released && !noReservation)
        || (order.cancellationVersion() >= 3 && !released)
        || (released && !itemsMatch)
        || (noReservation && !releasedItems.isEmpty())) {
