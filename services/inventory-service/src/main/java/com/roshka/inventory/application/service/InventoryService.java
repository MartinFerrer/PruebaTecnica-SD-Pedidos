...
public class InventoryService
	...
	public ProductSnapshot create(CreateProductUseCase.Command command) {
		...
		Product product = new Product(ids.get(), command.sku(), command.name(), new Stock(command.initialStock(), 0, 1),
				clock.instant());
	...
	public StockChange restock(UUID productId, RestockProductUseCase.Command command) {
		...
		StockChange result = change(after, before, command.movementId(), command.quantity(), command.reason());
		store.update(after);
	...
	public StockChange recount(RecountStockUseCase.Command command) {
		...
		Product after = before.withStock(next, clock.instant());
		StockChange result = change(after, before, ids.get(), command.stock() - before.stock().onHand(),
				command.reason());
	...
	private Product product(UUID id, boolean lock) {
		return store.find(id, lock).orElseThrow(() -> BusinessException.notFound("PRODUCT_NOT_FOUND"));
	}

	public static ProductSnapshot snapshot(Product product) {
		return new ProductSnapshot(product.productId(), product.sku(), product.name(), product.stock().onHand(),
				product.stock().reserved(), product.stock().available(), product.stock().version(),
				product.updatedAt());
	}

	private StockChange change(Product product, Product before, UUID movementId, long quantity, String reason) {
		return new StockChange(product.productId(), movementId, quantity, before == null ? 0 : before.stock().onHand(),
				product.stock().onHand(), product.stock().reserved(), product.stock().available(),
				product.stock().version(), reason);
	}
