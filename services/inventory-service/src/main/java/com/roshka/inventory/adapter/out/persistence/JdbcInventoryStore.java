...
public class JdbcInventoryStore implements InventoryStore {
	...
	public JdbcInventoryStore(JdbcClient db, ProductRowMapper products, StockMovementPersistenceMapper movements) {
		this.db = db;
	...
	public boolean skuExists(String sku) {
		return db.sql("SELECT count(*) FROM products WHERE sku=:sku").param("sku", sku).query(Long.class).single() > 0;
	}
	...
	public List<Product> findAll() {
		return db.sql("SELECT * FROM products ORDER BY product_id").query(products).list();
	}

	public void insert(Product p) {
		db.sql(INSERT_PRODUCT_SQL)
			.param("id", p.productId())
			.param("sku", p.sku())
			.param("name", p.name())
			.param("hand", p.stock().onHand())
			.param("at", java.sql.Timestamp.from(p.updatedAt()))
			.update();
	}

	public void update(Product p) {
		db.sql(UPDATE_PRODUCT_SQL)
			.param("hand", p.stock().onHand())
			.param("reserved", p.stock().reserved())
			.param("version", p.stock().version())
			.param("at", java.sql.Timestamp.from(p.updatedAt()))
			.param("id", p.productId())
			.update();
	}
	...
	public Optional<StockChange> movement(UUID id) {
		return db.sql("SELECT response FROM stock_movements WHERE movement_id=:id")
			.param("id", id)
			.query((rs, n) -> movements.fromJson(rs.getString(1)))
			.optional();
	}

	public void movement(String operation, Product before, Product after, StockChange result) {
		db.sql(INSERT_MOVEMENT_SQL)
			.param("id", result.movementId())
			.param("product", after.productId())
			.param("op", operation)
			.param("hand", after.stock().onHand() - (before == null ? 0 : before.stock().onHand()))
			.param("reserved", after.stock().reserved() - (before == null ? 0 : before.stock().reserved()))
			.param("version", after.stock().version())
			.param("response", movements.toJson(result))
			.update();
	}
