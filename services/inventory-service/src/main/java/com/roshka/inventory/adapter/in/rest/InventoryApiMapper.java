...
public class InventoryApiMapper {
  public CreateProductUseCase.Command toCommand(InventoryRequest.Create request) {
    return new CreateProductUseCase.Command(
        request.sku(), request.name(), request.initialStock());
  }

  public RestockProductUseCase.Command toCommand(InventoryRequest.Restock request) {
    return new RestockProductUseCase.Command(
        request.movementId(), request.quantity(), request.reason());
  }

  public RecountStockUseCase.Command toCommand(InventoryRequest.Recount request) {
    return new RecountStockUseCase.Command(
        request.productId(), request.stock(), request.expectedVersion(), request.reason());
  }
