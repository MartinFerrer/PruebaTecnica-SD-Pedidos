...
class ServiceIT {
  ...
  void propagatesHttpTraceToTransactionalOutbox() throws Exception {
    ...
    var response =
        HTTP.send(
            HttpRequest.newBuilder(URI.create(base + "/products")).header("Content-Type", "application/json")
                .header("Idempotency-Key", key).header("traceparent", trace).header("X-Correlation-Id", key).POST(
  ...
  void concurrentRestocksAreAdditiveAndMovementsAreUnique() throws Exception {
    ...
    try (var pool = java.util.concurrent.Executors.newFixedThreadPool(8)) {
      ...
      start.countDown();
      for (var task : tasks) assertThat(task.get().statusCode()).isEqualTo(201);
    }
  ...
  void stockMutationAndOutboxRollBackTogether() throws Exception {
    ...
    var db = app.getBean(org.springframework.jdbc.core.simple.JdbcClient.class);
    assertThat(
            db.sql("SELECT count(*) FROM stock_movements WHERE movement_id=:id").param("id", movement).query(Long.class)
                .single()).isZero();
    assertThat(
            db.sql("SELECT count(*) FROM message_outbox WHERE body LIKE :id").param("id", "%" + movement + "%")
  ...
  void manualReplayAfterCommitDoesNotReserveAgain() throws Exception {
    ...
    send("OrderCreated", order, 1, event, payload);
    org.awaitility.Awaitility.await().during(Duration.ofMillis(500)).atMost(Duration.ofSeconds(5))
        .untilAsserted(() -> assertThat(stock(id).get("reserved").asLong()).isEqualTo(2));
  }
  ...
  void cancellationBeforeCreationNeverReserves() throws Exception {
    String id = createProduct(10), order = UUID.randomUUID().toString();
    send(
        "OrderCancelled",
        order,
        2,
        UUID.randomUUID().toString(),
        java.util.Map.of("orderId", order));
    send(
        "OrderCreated",
        order,
        1,
    ...
    org.awaitility.Awaitility.await().atMost(Duration.ofSeconds(10)).untilAsserted(
            () -> {
              assertThat(
                      db.sql("SELECT state FROM reservations WHERE order_id=CAST(:id AS uuid)").param("id", order)
                          .query(String.class).optional()).contains("CANCELLED_BEFORE_RESERVATION");
  ...
  void allOrNothingReportsEveryShortage() throws Exception {
    String available = createProduct(10),
        empty = createProduct(0),
        missing = UUID.randomUUID().toString(),
        order = UUID.randomUUID().toString();
    send(
        "OrderCreated",
        order,
        ...
        java.util.Map.of(
            ...
            "items",
            java.util.List.of(
                java.util.Map.of("productId", available, "quantity", 1),
    ...
    org.awaitility.Awaitility.await().atMost(Duration.ofSeconds(10)).untilAsserted(
            () -> {
              var found =
                  db.sql(
                          "SELECT body FROM message_outbox WHERE event_type='StockRejected' AND body LIKE :id")
                      .param("id", "%" + order + "%").query(String.class).optional();
    ...
    String body =
        JSON.writeValueAsString(
            java.util.Map.ofEntries(
                ...
                java.util.Map.entry("aggregateId", order),
                java.util.Map.entry("aggregateVersion", version),
                java.util.Map.entry("occurredAt", java.time.Instant.now().toString()),
                java.util.Map.entry("producer", "order-service"),
                java.util.Map.entry("correlationId", order),
  ...
  void createReplayRestockAndVersionedRecount() throws Exception {
    ...
    var stale =
        request(
            ...
            "{\"productId\":\""
                + id
                + "\",\"stock\":20,\"expectedVersion\":1,\"reason\":\"RECOUNT\"}");
