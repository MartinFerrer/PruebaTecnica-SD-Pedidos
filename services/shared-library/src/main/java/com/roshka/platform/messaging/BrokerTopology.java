...
public class BrokerTopology {
  ...
  public Declarables topology() {
    ...
    for (String service : List.of("order", "inventory")) {
      ...
      List<String> types = service.equals("order")
          ? List.of("StockReserved", "StockRejected", "StockReleased")
    ...
    Queue audit =
        QueueBuilder.durable("inventory.products").quorum().ttl(86400000).maxLength(10000).build();
