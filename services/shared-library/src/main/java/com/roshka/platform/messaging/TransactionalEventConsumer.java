 ...
 *
 * <p>The callback remains in each service so that this technical module never depends on a domain
 * model. The class owns only envelope validation, inbox registration, context propagation and
 * retry/DLQ transfer.
 */
public abstract class TransactionalEventConsumer {
  ...
  protected TransactionalEventConsumer(
      JsonCodec json,
      JdbcClient db,
    ...
    try {
      ...
      tx.executeWithoutResult(
          status -> {
            ...
            int added =
                db.sql(
                        "INSERT INTO message_inbox(consumer_name,event_id) VALUES (:consumer,:id) "
            ...
            if (added != 0) {
              var context =
                  new MessageContext(
                      ...
                      requiredText(event, "traceparent"));
  ...
  private void transferFailure(Message message, Channel channel, RuntimeException failure)
      throws IOException {
    ...
    String destination =
        permanent || attempt >= RETRY_DELAYS_SECONDS.length
            ? deadLetterQueue
  ...
  private void validate(JsonNode event) {
    if (!event.isObject()
        ...
        || !event.hasNonNull("aggregateId")
        || !event.hasNonNull("eventType")
        || !event.hasNonNull("payload")
        || event.path("schemaVersion").asInt() != 1
        || event.path("aggregateVersion").asLong() < 1
    ...
    if (!requiredText(event, "traceparent").matches(
            "00-(?!0{32})[0-9a-f]{32}-(?!0{16})[0-9a-f]{16}-[0-9a-f]{2}")
        || requiredText(event, "correlationId").isBlank()
        || requiredText(event, "causationId").isBlank())
      throw new IllegalArgumentException("INVALID_CONTEXT");
    if (!requiredText(event, "aggregateId")
        .equals(requiredText(event.get("payload"), "orderId")))
      throw new IllegalArgumentException("ORDER_ID_MISMATCH");
  }

  protected static JsonNode required(JsonNode node, String field) {
    JsonNode value = node.get(field);
    if (value == null || value.isNull()) throw new IllegalArgumentException("MISSING_" + field);
    return value;
  }

  protected static String requiredText(JsonNode node, String field) {
    String value = required(node, field).stringValue();
    if (value == null || value.isBlank()) throw new IllegalArgumentException("INVALID_" + field);
    return value;
  }
}
