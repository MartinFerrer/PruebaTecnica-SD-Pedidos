 ...
 *
 * <p>The business-failure mapper is supplied by each service, so this technical component does not
 * depend on either bounded context or on their exception hierarchies.
 */
public final class RequestTransactions {
  ...
  public IdempotencyExecutor.Reply write(
        ...
        () -> {
          try {
            ...
            return result instanceof Result response
                ? executor.success(response.status(), response.body())
